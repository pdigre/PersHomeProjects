...
import no.esito.genova.io.antlr.GLexer;
...
public class GLexerSub extends GLexer {
    ...
    public GLexerSub(CharStream is) {
        super(is);
    ...
    public Token nextToken() {
        if (mode == MODE_CMT) {
            ...
            if (isTypes())
                ...
                mode = MODE_OUT;
            CommonToken t = new CommonToken((CharStream) input, GLexer.REM, Token.DEFAULT_CHANNEL, begin,
                    end - 1);
        ...
        if (mode == MODE_OUT) {
            ...
            int end = begin + l - 1;
            if (end > begin) {
                CommonToken t = new CommonToken((CharStream) input, GLexer.OUT, Token.DEFAULT_CHANNEL, begin,
                    end - 1);
                t.setLine(input.getLine());
                t.setText(null);
                t.setCharPositionInLine(input.getCharPositionInLine());
