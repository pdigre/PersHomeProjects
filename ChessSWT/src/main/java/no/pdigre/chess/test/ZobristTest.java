...
import no.pdigre.chess.polyglot.BookEntry;
import no.pdigre.chess.polyglot.BookEntry.BookMove;
import no.pdigre.chess.polyglot.Polyglot;
...
public class ZobristTest {
    ...
    public static void assertMoves(String fen, String moves_expected) {
        long key = ZobristKey.getKey(new StartGame(fen));
        BookEntry entry = Polyglot.book.get(new ZobristKey(key));
        StringBuffer sb=new StringBuffer();
        for (BookMove move : entry.moves){
            if(sb.length()>0)
                sb.append(" ");
        ...
        assertEquals(moves_expected,sb.toString());
    }
