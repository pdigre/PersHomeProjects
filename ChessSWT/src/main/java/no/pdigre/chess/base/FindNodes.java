            ...
            case INode.KING:
                ...
                if (white) {
                    if ((castling & INode.NOCASTLE_BLACKQUEEN) == 0)
                        castlingQueen(adder, board, from, INode.BLACK_ROOK, white);
                    if ((castling & INode.NOCASTLE_BLACKKING) == 0)
                        castlingKing(adder, board, from, INode.BLACK_ROOK, white);
                } else {
                    if ((castling & INode.NOCASTLE_WHITEQUEEN) == 0)
                        castlingQueen(adder, board, from, INode.ROOK, white);
                    if ((castling & INode.NOCASTLE_WHITEKING) == 0)
                        castlingKing(adder, board, from, INode.ROOK, white);
                }
                break;
        }
    }
