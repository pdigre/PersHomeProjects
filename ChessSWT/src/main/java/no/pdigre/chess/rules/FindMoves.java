...
public class FindMoves {
	...
	final public static void addMovesForPiece(IAdder adder,
			int[] board, int piece) {
		...
		case IMove.PAWN:
			...
			pawnBeat(adder, board, from, enpassant, fwd + BaseMoves.LEFT,
					white);
			pawnBeat(adder, board, from, enpassant, fwd + BaseMoves.RIGHT,
					white);
			break;
		case IMove.KNIGHT:
	...
	final private static void addSlider(IAdder imoves, int[] board,
			int[][] moves, boolean white) {
		for (int[] slide : moves) {
			for (int i = 0; i < slide.length
					&& add(imoves, board, slide[i], white); i++) {
				// not
			}
		}
	}

	final private static void addSimple(IAdder imoves, int[] board,
			int[] moves, boolean white) {
		for (int to : moves)
	 ...
	 * @return
	 */
	final private static boolean add(IAdder moves, int[] board, int to,
			boolean white) {
		int victim = board[to];
		if (victim == 0)
			moves.move(to);
	...
	final private static void pawnBeat(IAdder moves, int[] board, int from,
			int enpassant, int leftright, boolean white) {
	...
	final private static void pawnForward(IAdder moves, int[] board, int from,
			int mv, boolean white) {
		int to1 = from + mv;
		if (inside(to1, from)) {
			if (board[to1] == 0) {
	...
	final private static void castlingKing(IAdder moves, int[] board, int from,
			int rook, boolean white) {
		int home = home(white);
		if (board[home + 5] == 0 && board[home + 6] == 0
				&& board[home + 7] == rook) {
	...
	final private static void castlingQueen(final IAdder moves,
			final int[] board, int from, int rook, boolean white) {
		int home = home(white);
		if (board[home + 3] == 0 && board[home + 2] == 0
				&& board[home + 1] == 0
				&& board[home + 0] == rook) {
