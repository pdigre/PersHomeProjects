...
public class StartGame implements IMove {
	...
	@Override
	public int[] getBoard() {
		return FEN.array2board(getPieces());
	}
...
//	}
	...
	public int[] getPieces() {
		...
		for (int i = 0; i < board.length(); i++) {
			...
			if (c == '/') {
				...
				int type = FindMoves.getPieceType(c).bitmap | ((x + y)<<12);
				list.add(type);
				x++;
			}
		}
	...
	public int getCastlingState() {
		return (castling.contains("K") ? 0 : IMove.NOCASTLE_WHITEKING)
				| (castling.contains("Q") ? 0 : IMove.NOCASTLE_WHITEQUEEN)
				| (castling.contains("k") ? 0 : IMove.NOCASTLE_BLACKKING)
				| (castling.contains("q") ? 0 : IMove.NOCASTLE_BLACKQUEEN);
	}
