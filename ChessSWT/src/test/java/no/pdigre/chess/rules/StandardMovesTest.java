...
public class StandardMovesTest {
	...
	public static String getMoves(String from, StartGame start) {
		EvalMove eval = new EvalMove(start);
		int piece = start.getBoard()[FEN.text2pos(from)];
		StringBuffer sb = new StringBuffer();
		sb.append(PieceType.types[Move.getPos(piece)].fen);
		for (Integer pos : eval.getLegalMoves(piece)) {
			sb.append(" ");
			sb.append(FEN.pos2text(pos));
		}
		return sb.toString();
	}
