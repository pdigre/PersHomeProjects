...
import static no.esito.genova.io.antlr.GParser.*
    	...
    	switch node.token.type {
	        ...
	        case PROG:
				...
				class «clazzname» extends GenovaBase{ 
					X«getRoot» «fixName(getRoot.toLowerCase)»
					override call(){
					 	«walk»«close»
					 	«"'''«context.toString»'''"»
					 }
            ...
            case ITERATE:{
				var iterator=txt(1).toString   
				          	
	        	'''
	        	for(«fixName(iterator.toLowerCase)»:«fixName(deduceParent(iterator).toLowerCase)».iterate«iterator»){
	        		«walk(1)»«close»
	        	}
	        	'''
            }
