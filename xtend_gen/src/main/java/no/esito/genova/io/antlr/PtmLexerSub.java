...
public class PtmLexerSub extends PtmLexer {
	...
	public PtmLexerSub(CharStream is) {
		super(is);
	...
	public Token nextToken() {
		if (mode == MODE_CMT) {
			...
			if (isTypes())
				...
				mode = MODE_OUT;
			CommonToken t = new CommonToken((CharStream) input, PtmLexer.REM,
		...
		if (mode == MODE_OUT) {
			...
			int end = begin + l - 1;
			if (end > begin) {
				CommonToken t = new CommonToken((CharStream) input, PtmLexer.OUT,
						Token.DEFAULT_CHANNEL, begin, end - 1);
				t.setLine(input.getLine());
				t.setText(null);
				t.setCharPositionInLine(input.getCharPositionInLine());
	...
	private int seekOUT() {
		for (int i = 1;; i++) {
			...
			case '@':
				...
				if ("@reserved".equals(rest) || "@types".equals(rest)
						|| "@template".equals(rest))
					return i;
				continue;
			case '\n':
				isFile = false;
